/**
 * Feed page loader — returns the next page of post JSON for the
...
import { listChannelPosts, listGroupFeed, serializePost } from "@/server/posts";
...
export async function GET(req: Request) {
  ...
  const page = channelId
    ? await listChannelPosts({ channelId, viewerId, cursor })
    : await listGroupFeed({ groupId: groupId!, userId: viewerId, cursor });

  return NextResponse.json({
    items: page.items.map((p) => serializePost(p, viewerId)),

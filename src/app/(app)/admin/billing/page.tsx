...
export default async function AdminBillingPage() {
  ...
  const plans = (["STARTER", "PRO", "BUSINESS"] as Plan[]);
    ...
    <div className="space-y-8">
      ...
      <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-0.5 text-sm text-muted-foreground">{cfg.price}</p>
          </div>
          <div className="text-right">
            <span className={cn(
              "inline-flex rounded-full px-2.5 py-1 text-xs font-semibold",
              tenant.planStatus === "ACTIVE" ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400"
                : tenant.planStatus === "TRIAL" ? "bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400"
      ...
      <div>
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-lg font-bold">{pc.price}</p>
                  </div>
                  {isCurrent && (
        ...
        <p className="mt-3 text-center text-xs text-muted-foreground">
          ...
          <a href="mailto:support@nadi.app" className="text-primary hover:underline">support@nadi.app</a>{" "}
          to upgrade.
        </p>
      </div>
    </div>
